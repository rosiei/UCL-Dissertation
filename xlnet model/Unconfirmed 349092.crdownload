...
def main():
    ...
    if not args.do_train and not args.do_test and not args.do_eval:
    ...
    tokenizer = XLNetTokenizer.from_pretrained(args.xlnet_model, do_lower_case=True)
    ...
    if args.trained_model_dir: # load in fine-tuned (with cloze-style LM objective) model
        ...
        model = XLNetForSequenceClassification.from_pretrained(args.trained_model_dir, state_dict=previous_state_dict, num_labels=num_labels)
    else:
    ...
    if args.do_test:
        if args.mode[:7] == "dirctr_":
            ma_test_examples, clean_test_examples, toxic_test_examples = ma_processor.get_direct_control_test_examples(args.data_dir)
            test_examples_seq = [ma_test_examples, clean_test_examples, toxic_test_examples]
        ...
        for set_idx, test_examples in enumerate(test_examples_seq):
            ...
            for input_ids, input_mask, segment_ids, label_ids, guids in tqdm(test_dataloader, desc="Testing"):
                ...
                with torch.no_grad():
                    ...
                    # logits = model(input_ids = input_ids,token_type_ids = segment_ids, attention_mask = input_mask)[1]
            ...
            #calc roc score
            # roc_score=roc_auc(y_predict, label_ids)
            classification_rep =metrics.classification_report(y_pred=np.array(y_predict),y_true=np.array(y_true))
