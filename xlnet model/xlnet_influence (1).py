...
def hv(loss, model_params, v):
    grad = autograd.grad(loss, model_params, create_graph=True, retain_graph=True, allow_unused=True)
    Hv = autograd.grad(grad[1:], model_params, grad_outputs=v,allow_unused=True)
    return Hv
...
def get_inverse_hvp_lissa(v, model, param_influence, train_lissa_loader, args):
    ...
    for i in range(args.lissa_repeat):
        ...
        for j in range(args.lissa_depth):
            ...
            hvp = hv(train_loss.loss, param_influence, cur_estimate)
            cur_estimate = [_a + (1 - args.damping) * _b - _c / args.scale
                            for _a, _b, _c in zip(v[1:], cur_estimate, hvp[1:])]
    ...
    # inverse_hvp_dir= 'ihvp'
    ihvp_filename = f"ihvp.pkl"
    pickle.dump(inverse_hvp_dir, open(os.path.join(args.inverse_hvp_dir, influence_filename), "wb"))       
...
def main():
    ...
    # Prepare model
    model = XLNetForSequenceClassification.from_pretrained(args.trained_model_dir, num_labels=num_labels)
    model.to(device)
...
#     if (not any(fr in n for fr in frozen)):
    ...
    for tmp_idx, (input_ids, input_mask, segment_ids, label_ids, guids) in enumerate(test_dataloader):
        ...
        ################
        #save inverse hvp
        # inverse_hvp_dir= 'inverse_hvp_saved'
        # inverse_hvp_filename = f"inverse_hvp.pkl"
        # pickle.dump(inverse_hvp_dir, open(os.path.join(args.inverse_hvp_dir, influence_filename), "wb"))
